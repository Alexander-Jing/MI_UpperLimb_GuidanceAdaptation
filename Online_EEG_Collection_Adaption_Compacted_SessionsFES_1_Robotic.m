...
% 运动想象基本参数设置
subject_name = 'Jyt_test_0822_online';  % 被试姓名
sub_offline_collection_folder = 'Jyt_test_0725_offline_20240725_194842721_data';  % 被试的离线采集数据
subject_name_offline =  'Jyt_test_0725_offline';  % 离线收集数据时候的被试名称
% session 大于1时候要改动的部分
% 注意，由于设备问题，建议在session_idx为4之前重启下matlab，防止出现后面的中断
session_idx = 3;  % session index数量，如果是1的话，会自动生成相关排布
foldername_Sessions = 'Jyt_test_0822_online_20240822_113357877_data';  % 当session大于1的时候，需要手工修正foldername_Sessions
...
% 电刺激强度设置
StimAmplitude_1 = 5;  % MI1 肩关节的电刺激幅值测试（mA）
StimAmplitude_2 = 7;  % MI2 手部分的幅值设置（mA）
...
% MovingCommand 完成动作之后移动的命令 PreparingCommand 准备下一步动作的指令
function [MI_AOTime_Moving, MI_AOTime_Preparing, MovingCommand, PreparingCommand] = RobotCommand(Train_Thre_Global_Flag, Task_now, Task_next)
