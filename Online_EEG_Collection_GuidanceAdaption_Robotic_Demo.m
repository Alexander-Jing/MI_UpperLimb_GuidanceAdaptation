...
% 运动想象基本参数设置
subject_name = 'Jyt_test_0514_online';  % 被试姓名，这里使用我的名字暂时用于测试
sub_offline_collection_folder = 'Jyt_test_0513_offline_20240513_171257133_data';  % 被试的离线采集数据
subject_name_offline =  'Jyt_test_0513_offline';  % 离线收集数据时候的被试名称
MotorClass = 2; % 运动想象动作数量，注意这里是纯设计的运动想象动作的数量，不包括空想idle状态
%MotorClassMI = 2;  % 如果是单运动想象任务的话，那就直接指定任务就好了
original_seq = [1,2, 0,0, 1,2, 0,0, 1,2, 1,2, 0,0];  % 原始序列数组
%original_seq = [1, 2, 0];  % 原始序列数组
training_seqs = 4;  % 训练轮数
...
trial_random = 0;  % 用于判断是否进行随机训练顺序的参数 false 0， true 1
feedback_set = 1;  % 用于设置反馈的flag，如果设置成1的话，会强制将Train_Thre_Global_Flag设置为1

% 通信设置，用于连接本地python的算法部分
%ip = '172.18.22.21';
ip = '127.0.0.1';
...
% 关于训练时刻的操作和flag的处理
Train_Thre = 0.45;  % 用于衡量后续是keep还是adjust的阈值
...
%         mu_suppressions_normalized = [mu_suppressions_normalized, mu_suppression_normalized]; 
   ...
   if Timer == 30 && Trials(AllTrial) > 0
       ...
       if feedback_set == 1
          if Train_Thre_Global_Flag == 0
              Train_Thre_Global_Flag = 1;  % 人为设置，强制将数值设置为1
          end
       end
   ...
   %% 休息阶段，确定下一个动作
    ...
    if Trials(AllTrial)>0 && Timer == 45  %开始休息
        Trigger = 7;
        sendbuf(1,1) = hex2dec('02') ;
        sendbuf(1,2) = hex2dec('00') ;
        sendbuf(1,3) = hex2dec('00') ;
    ...
    %% 最后的各个数值复位
    % 空想任务想象18s，到第18s之后开始休息，到第30s就结束任务，留下10s用于更新模型
    if Timer == 30 && Trials(AllTrial)==0  %结束休息，准备下一个
        % 存储相关的EI指标和mu节律能量的数据
    ...
    if Trials(AllTrial)>0 && Timer == (45 + RestTimeLen)  %结束休息
        % 存储相关的EI指标和mu节律能量的数据
        SaveMIEngageTrials(EI_indices, mu_powers, mu_suppressions, subject_name, foldername, config_data, EI_index_scores, resultsMI, ...
            MI_Acc, MI_Acc_GlobalAvg, TrialData_Processed);
        % 计时器清0
