...
% 运动想象基本参数设置
subject_name = 'Jyt_test_0601_offline';  % 被试姓名
TrialNum = 3*40;  % 设置采集的数量
%TrialNum = 3*3;
...
while(AllTrial <= TrialNum)
    if Timer==0  %提示专注 cross
        ...
        fwrite(UnityControl,sendbuf);       
        AllTrial = AllTrial + 1;
        % 确定下休息时间的长度，中间隔了40个trial（10分钟）最好休息下，休息一定时间
        if mod(AllTrial,40)==0
            RestTimeLenBaseline = 60*3;
            disp(["40个trial了，休息3分钟"])
        else
    ...
    if Timer==2
        ...
        if Trigger > 0
            Trigger = 9;  % 记录此时有AO和电刺激时候的数据，防止后续的数据处理有影响
        else
            Trigger = 0;
        end
    end
    ...
    if Timer >= 6 && Timer < 10 && RandomTrial(AllTrial)> 0
        Trigger = RandomTrial(AllTrial);  % 动作（Idle, MI1, MI2）
    ...
    % 第8s开始取512的Trigger~=6的MI的窗口，数据处理并且进行分析，这里距离电刺激还间隔了1s，防止出现电刺激的影响
    ...
    if Timer==10 && RandomTrial(AllTrial)> 0  %开始休息
        Trigger = 7;
    ...
    if Timer==6 && RandomTrial(AllTrial)==0  %开始休息
        Trigger = 7;
        sendbuf(1,1) = hex2dec('02') ;
        sendbuf(1,2) = hex2dec('00') ;
        sendbuf(1,3) = hex2dec('00') ;
