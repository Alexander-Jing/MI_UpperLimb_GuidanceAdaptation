...
% 运动想象基本参数设置
subject_name = 'Wyx_0923_compare_online_test';  % 被试姓名
sub_offline_collection_folder = 'Wyx_0923_compare_offline_20240929_203055611_data';  % 被试的离线采集数据
subject_name_offline =  'Wyx_0923_compare_offline';  % 离线收集数据时候的被试名称
% session 大于1时候要改动的部分
% 注意，由于设备问题，建议在session_idx为4之前重启下matlab，防止出现后面的中断
session_idx = 1;  % session index数量，如果是1的话，会自动生成相关排布
foldername_Sessions = '';  % 当session大于1的时候，需要手工修正foldername_Sessions
