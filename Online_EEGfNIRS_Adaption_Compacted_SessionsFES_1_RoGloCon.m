...
% 运动想象基本参数设置
subject_name = 'Wzh_compare_online';  % 被试姓名
sub_offline_collection_folder = 'Wzh_compare_offline_20241002_161200262_data';  % 被试的离线采集数据
subject_name_offline =  'Wzh_compare_offline';  % 离线收集数据时候的被试名称
% session 大于1时候要改动的部分
% 注意，由于设备问题，建议在session_idx为4之前重启下matlab，防止出现后面的中断
session_idx = 9;  % session index数量，如果是1的话，会自动生成相关排布
foldername_Sessions = 'Wzh_compare_online_20241002_165922290_data';  % 当session大于1的时候，需要手工修正foldername_Sessions
...
% 动画显示设置
AO_Dynamic = 0;  % 用于动画显示的设置数值，如果设置为1，表明显示动画，否则是0，这里为了对照实验，设置为0
    ...
    if ((Timer-2) >1) && Trials(AllTrial_Session)> 0 && Timer <= MI_preFeedBack
        ...
        if Timer == MI_preFeedBack
            ...
            if cls_ == (Trigger+1)  % 如果最大值对应的是trigger+1，那么就是投票结果显示分类正确
                ...
                disp(['投票没有达到条件，', '预测值：', num2str(cls_-1), '，实际数值：', num2str(Trigger)]);
            end
        end
