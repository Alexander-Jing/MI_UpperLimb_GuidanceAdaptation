...
% 运动想象基本参数设置
subject_name = 'Jyt_test_0423_onlineBasline';  % 被试姓名
sub_offline_collection_folder = 'Jyt_test_0422_offline_20240422_152139532_data';  % 被试的离线采集数据
subject_name_offline =  'Jyt_test_0422_offline';  % 离线收集数据时候的被试名称
...
Train_Thre_Global = Train_Thre_Global_FeasibleInit(2,2);  % 用于并且调整的针对全局均值的可行-最优策略的阈值设定
...
while(AllTrial <= TrialNum)
    ...
    % 开始动态想象
    if Timer > 2 && (mod(Timer-2, 4)==2 || mod(Timer-2, 4)==3 || mod(Timer-2, 4)==0) && Trials(AllTrial)> 0 && Timer < 20
        disp(['开始训练']);
        Trigger = Trials(AllTrial);
        rawdata = TrialData(:,end-512+1:end);  % 取前一个512的窗口
        rawdata = rawdata(2:end,:);
...
%         mu_suppressions_normalized = [mu_suppressions_normalized, mu_suppression_normalized]; 
   ...
   if Timer == 18 && Trials(AllTrial) > 0
       Trigger = 8;
       if Train_Thre_Global_Flag==1  % 如果想对了
           if Trials(AllTrial) > 0  % 运动想象任务
